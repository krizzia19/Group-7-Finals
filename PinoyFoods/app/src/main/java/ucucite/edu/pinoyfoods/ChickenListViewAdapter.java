...
public class ChickenListViewAdapter extends BaseAdapter{

    //variables
    Context mContext;
    LayoutInflater inflater;
    List<ChickenModel> modellist;
    ArrayList<ChickenModel> arrayList;

    //constructor
    public ChickenListViewAdapter(Context context, List<ChickenModel> modellist) {
        mContext = context;
        this.modellist = modellist;
        inflater = LayoutInflater.from(mContext);
        this.arrayList = new ArrayList<ChickenModel>();
    ...
    public void filter(String charText){
        ...
        if (charText.length()==0){
            ...
            for (ChickenModel chickenModel : arrayList){
                if (chickenModel.getTitle().toLowerCase(Locale.getDefault())
                        .contains(charText)){
                    modellist.add(chickenModel);
                }
            }
        }
